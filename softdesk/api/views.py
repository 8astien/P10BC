...
class ProjectAPIView(APIView):
    ...
    permission_classes = [
        IsAuthenticated & IsAuthor | IsContributor | IsAdminUser
    ]
    ...
    def post(self, request, format=None):
        ...
        elif isinstance(data['contributors'], int):
...
class IssueAPIView(APIView):
    ...
    permission_classes = [
        IsAuthenticated & IsContributor | IsAuthor | IsAdminUser
    ]
    pagination_class = LimitOffsetPagination
        ...
        if "contributor" not in request.data:
            ...
            try:
                contributor = User.objects.get(
                    username=contributor_username).id
            except User.DoesNotExist:
                return Response({"message": "username doesn't exist."})
        data = request.data.copy()
        data.update({
